...
class Master extends DBConnection
{
	...
	function save_message()
	{
		...
		if ($save) {
			...
			if (empty($id))
				...
				$resp['msg'] = "Mensaje actualizado correctamente";
		} else {
			$resp['status'] = 'failed';
	...
	function delete_message()
	{
		...
		if ($del) {
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success', "Mensaje ha sido eliminado correctamente");
		} else {
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
	...
	function save_category()
	{
		...
		if ($check->num_rows > 0) {
			$resp['status'] = 'failed';
			$resp['msg'] = "Nombre de categoría ya existe";
		} else {
			$save = $this->conn->query($sql);
			if ($save) {
				$rid = !empty($id) ? $id : $this->conn->insert_id;
				$resp['status'] = 'success';
				if (empty($id))
					$resp['msg'] = "Categoría ha sido agregada correctamente.";
				else
					$resp['msg'] = "La Categoría ha sido actualizada correctamente.";
			} else {
				$resp['status'] = 'failed';
				$resp['msg'] = "Ocurrió un error";
				$resp['err'] = $this->conn->error . "[{$sql}]";
	...
	function delete_category()
	{
		...
		if ($del) {
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success', "Categoría ha sido eliminada correctamente.");
		} else {
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
	...
	function save_service()
	{
		...
		if ($check > 0) {
			...
			if ($save) {
				...
				$resp['status'] = 'success';
				if (empty($id))
					$resp['msg'] = "Servicio ha sido agregado correctamente";
				else
					$resp['msg'] = "Servicio ha sido actualizado correctamente";
			} else {
				$resp['status'] = 'failed';
				$resp['msg'] = "Ocurrió un error";
				$resp['err'] = $this->conn->error . "[{$sql}]";
	...
	function delete_service()
	{
		...
		if ($del) {
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success', "Servicio ha sido eliminado correctamente");
		} else {
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
	...
	function save_appointment()
	{
		...
		if ($slot_taken >= $this->settings->info('max_appointment')) {
			$resp['status'] = 'failed';
			$resp['msg'] = "Lo sentimos, el calendario de citas ya está lleno.";
		} else {
			$save = $this->conn->query($sql);
			if ($save) {
				$rid = !empty($id) ? $id : $this->conn->insert_id;
				...
				$resp['status'] = 'success';
				if (empty($id))
					$resp['msg'] = "Los nuevos detalles de la cita se agregaron correctamente.</b>.";
				else
	...
	function update_appointment_status()
	{
		...
		if ($del) {
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success', "El estado de la solicitud de cita se actualizó exitosamente.");
		} else {
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
